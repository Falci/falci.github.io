...
- Directed the roadmap for hns.id's Ethereum/EVM smart contracts and Layer 2 (Optimism) integrations, guiding the smart contract engineering team's use of Solidity and Web3.js to deliver DeFi and NFT (ERC-721) features.
- Led backend development of the main Namebase application, a Handshake marketplace, crypto exchange, and auction platform, using Node.js and TypeScript to design the REST APIs, versioning strategy, rate limiting, and real-time (SSE) layers.
...
- Designed the platform's data layer architecture across PostgreSQL, Redis, Kafka, and DynamoDB, with careful schema design for performance at scale, overseeing the Go services that implemented it.
- Architect and maintain the hns.id frontend in TypeScript, React, and Next.js, integrating TanStack Query and Storybook for a component-driven UI deployed on Vercel, actively developed and extended alongside the backend and protocol roadmap.
